...
func _ready() -> void:
	...
	if hb:
		...
		# --- Collision: force hitbox to a known layer/mask ---
		for i in range(1, 33):
			hb.set_collision_layer_value(i, false)
			hb.set_collision_mask_value(i, false)

		hb.set_collision_layer_value(2, true)  # boxes live on Layer 2
		hb.set_collision_mask_value(1, true)   # see sabers on Layer 1
